...
// 2. Create the button element for the dropdown.
const studentDropdownBtn = document.createElement('button');
studentDropdownBtn.id = 'studentDropdownBtn';
studentDropdownBtn.textContent = 'Select a Student';
myVideoNavBar.appendChild(studentDropdownBtn);

// 3. Create the dropdown menu container.
const studentDropdownMenu = document.createElement('div');
studentDropdownMenu.id = 'studentDropdownMenu';
studentDropdownMenu.classList.add('student-dropdownMenu');
myVideoNavBar.appendChild(studentDropdownMenu);

// 4. Generate student links dynamically and add them to the dropdown menu container.
const students = ['Alice', 'Bob', 'Charlie', 'David']; // Your list of students here
students.forEach(student => {
    ...
    studentDropdownMenu.appendChild(studentLink);
});

// 5. Attach the main div (myVideoNavBar) to the body or another desired parent element.
document.body.appendChild(myVideoNavBar);

// 6. Add event listeners and the required functions as before.
studentDropdownBtn.addEventListener('click', () => {
    if (studentDropdownMenu.style.display === 'none' || studentDropdownMenu.style.display === '') {
        studentDropdownMenu.style.display = 'block';
    } else {
        studentDropdownMenu.style.display = 'none';
    }
});

window.addEventListener('click', (event) => {
    if (!studentDropdownBtn.contains(event.target) && !studentDropdownMenu.contains(event.target)) {
        studentDropdownMenu.style.display = 'none';
    }
});

function selectStudent(studentName) {
    studentDropdownBtn.textContent = studentName;
    studentDropdownMenu.style.display = 'none';
}
...
document.getElementById('calculatorDiv').addEventListener('mousedown', (e) => {
    isDragging = true;
    
    const rect = calculatorDiv.getBoundingClientRect();
    const paddingX = parseInt(getComputedStyle(calculatorDiv).paddingLeft, 10);
    const paddingY = parseInt(getComputedStyle(calculatorDiv).paddingTop, 10);

    dragOffset = {
        x: e.clientX - rect.left + (isNaN(paddingX) ? 0 : paddingX),
        y: e.clientY - rect.top + (isNaN(paddingY) ? 0 : paddingY)
    };
...
function onMouseMove(e) {
    ...
    calculatorDiv.style.left = x + 'px';
    calculatorDiv.style.top = y + 'px';
}
...
function decodeBase85(input) {
    ...
    for (let i = 0; i < input.length; i++) {
        buffer = (buffer * BASE) + BASE85_CHARS.indexOf(input.charAt(i));
        count++;

        if (count === 5) {
            for (let j = 0; j < 4; j++) {
                output = String.fromCharCode(buffer & 0xFF) + output;
                buffer = buffer >> 8;
            }
...
function ParseVideoURLData(data) {
    try{
        // Split the data string by underscores to get individual items
        var items = data.split('_');

        // Remove the last empty string caused by the trailing underscore
        items.pop();

        // For each item, extract the Category and Percent
        var arr = [];
...
var MathrowsGlobal = [];
var VerbalrowsGlobal = [];
var ReadingrowsGlobal = [];
var ScienceRowsGlobal = [];

//function PullURLData(){
    var url = window.location.href;
    var urlSplit = url.split('/join/');
    var urlData =  urlSplit[1];
    var Data = decodeBase85(urlData);
    var DataSplit = Data.split('%');
    var CurrentTest = DataSplit[0];
    var MathrowsGlobal = DataSplit[1];
    var VerbalrowsGlobal = DataSplit[2];
    var ReadingrowsGlobal = DataSplit[3];
    var ScienceRowsGlobal = DataSplit[4];
    MathrowsGlobal = ParseVideoURLData(MathrowsGlobal);
    VerbalrowsGlobal = ParseVideoURLData(VerbalrowsGlobal);
    ReadingrowsGlobal = ParseVideoURLData(ReadingrowsGlobal);
    ScienceRowsGlobal = ParseVideoURLData(ScienceRowsGlobal);
//}
//Placeholder for AI Images

console.log('jfsjpkfjkgkfsjglkfjfklsjklgfk')
console.log(MathrowsGlobal)
  
// state variables
let imageDataUrl = null;
let imageText = "Hello World";
let currentLocalTopicForQuestions = '';
let currentLocalQuestionSentiment = '';
let chartSVG = 0;
let chartDataGlobal = 0;
let imgDataUrl = null;
let imageURLForQuestion = '';
let currentShaCode = '';
let currentQuestionInfo = '';
let currentPhaseInQuestion = 0;
let currentTest = 'SAT'
let currentType = 'Practice'
let correctAnswerChoicePractice = ''
let explanation = ''
let QuizNumber = 0
let QuestionNeedsReview = false




let TestURL = 'aA9L*bZ>G~B_&jKEj2S=VP|D?P+@a(a%E&7Ur=FbbaO2*Lug@gL2hIqaA<QNUq^Cba%C+rX=8MCX=fl%Xk{QmEihkHbZKvHMs;puMsIQ;X>MmAQe|OeXDu*aX>Mt5WMy(~WFS>;XCOgta&&2Kb#QQREihkGb7^mGWoTt?Z*6dLVQe5nX>?_BFkdB1ZgVX)a%ppKZ*6d4Ze$>1a&sVIAaG%6S8sA;EjKh@Wo~n2Ze$>IZge1FWo2z;AYo^6R%LQxEj2b@X>V?GZgXjLUsQ5ob1gYBaA;{`Z)b90UrBCeaxFG9Wps39X>%ZAbY*fNQ)q8;EjBk_Wo~qHa$#p}Ze$>4Wpf~5bY*U1UsGjnaxFMEZ*pX1VQ^?5a$#q3WFT;1Wgua0bY*U1UsGjnZY?)6VRUJ4Zew(HG+$74Zged<Wo2z;AYo^6Zf|vNVQyp~Z*^`UP;zf>EjBP;bairNbaHiLWo#gGa$#(2Fket%WpgbzY+-eCVQe5{Wo~q7AZ2rNAYpD~bZKF1b8}^GZ*DDRI5uBSX>V>VX=7n@Z)9m^Ffd<DWpXVsXK8bEWFT^7EFfWSX>V<FEFfuQbZKvHUqoqRb1gVHWo~0-b7gLHX>MmAVRB(~AaiAKAYpD~ZfS03Z*5{}H85X8Xk~LPbaHQSL2z$#EjBP;Y-Mz1av)@7Wh@|XWp-(EWGo<ZUnN0gWi2r{Z)|mKEihkJWpr|RZ*FgGRB~x&b1g7mZf9&|RB~xyEi^P=V{Bz}Uqfkfb1gDCZf9&|GcjL5Z*+4lav*SQVRUq5Fke$+bYU$wXCP!@VPt7;H(yd^Z*DC)Z*p{LaB^>OVQyp~bZKuOF<(+)ba^c|Vrgt?a&KZ`H8NjNZgg`kWpZO>EihkDZEs{{VQyp~X<=?4AZ=x2WnpeCH8o#NX=iRNAY^58ZDnqBaAk67FkeM@ZY?n|X>)0BAY^HFWprs{d2V!QGcsROX=8IOWO8A2Ur}{ob1gM6X=QG7W@c$)AY*T3XlZU|O<{CnEj2J-X>V?Gb#7yHHaA~JcXVhiAZK!KZe?L|WFTy5Y#?E7Zggp3aBps9FkeM@Y;!F&WNBkzWFT^3b0A@EZe?zCMR;&;EjTk@WMyn~XCQ5FV{~b5b98cbUqf$hb1gJ5ZDM6|cpz?dZE$R5IbTC>ZF4O)Z(?j^WFT;IUsrE(b1gM9bZKvHWpQ<3Aa7<MbY*RGUsHK=b1gSBbZKvHWpQ<3VQFI^XJuk?XCPs0Y<6jGF<(<}WpgbyUrcFkb1gJ8X>@62adlyAUrBCdb1gM6b7^mGaB^jHFkeM@b!9CzAa-GFY;|;HL1J@nQ$bWE'
let QuizQuestionLengths =  [['Numbers And Opperations', 11], ['Properties Of Numbers', 15], ['Lines', 12], ['Cordinate Geometry', 9], ['Probability', 10], ['Passages And Questions Modifications', 3], ['Locators', 2], ['Trends In Tables And Figures', 2], ['Extrapolation And Estimation', 1], ['Data Bridge', 2], ['Data Full Sentence', 1], ['Cannot Be Determined', 1], ['Equations', 1], ['Mixing', 1], ['Scatter Plot', 1], ['Inference Question', 1], ['Method Table', 1], ['Method Figures', 1], ['GenericLabels', 1], ['Inverse Trends', 1], ['OutsideKnowledge', 7], ['Exponents And Radicals', 16], ['Percent', 20], ['Exponential Growth', 17], ['Rates', 21], ['Ratio And Proportion', 15], ['Expression', 15], ['Construction Models', 14], ['Manipulating Solving Equations', 32], ['More Equation Solving Strategies', 18], ['Inequalities', 16], ['Word Problems', 23], ['Minimum And Maximum Word Problems', 17], ['Lines Questions', 15], ['Interpreting Linear Models', 16], ['Functions', 32], ['Quadratics', 19], ['Synthetic Division', 14], ['Complex Numbers', 15], ['Absolute Value', 15], ['SystemsOfEquations', 24], ['Angles', 14], ['Triangle', 29], ['Circle', 14], ['Trig', 15], ['Reading Data', 13], ['Probability', 15], ['Statistics1', 15], ['Statistics2', 15], ['Volume', 14], ['Adding Deleting Revising', 8], ['Sentence And Paragraph Order', 5], ['lnfographics', 2], ['Shorter Is Better', 5], ['Diction Idioms And Register', 5], ['Combining Separating Sentences', 10], ['Transitions', 10], ['Non Essential And Essential Clauses', 9], ['Colons And Dashes', 5], ['Apostrophes Plural Vs Possessive', 15], ['Pronoun And Noun Agreement', 10], ['Verbs Agreement And Tense', 8], ['Word Pairs And Comparisons', 5], ['Parallel Structure', 10], ['Dangling And Misplaced Modifiers', 5], ['Relative Pronouns', 5], ['Vocabulary In Context', 15], ['Pronouns And Compression Nouns', 11], ['TheBigPicture', 5], ['Literal Comprehension', 7], ['Reading For Function', 7], ['Text Completion', 6], ['Supporting And Undermining', 8], ['Graphs And Charts', 4], ['Paired Passages', 4]]

let currentQuizNumbers = []

function setCurrentQuizNumbers(arr){
    currentQuizNumbers = arr
}

function QuizQuestionLengthsFunction(currTopic = currentLocalTopicForQuestions){
    let currentQuizNumbers = []
    console.log('currTopic',currTopic)
    for(let i = 0; i < QuizQuestionLengths.length; i++){
        if(QuizQuestionLengths[i][0].toLowerCase().replaceAll(' ','') == currTopic.toLowerCase().replaceAll(' ','')){
            for(let j = 0; j < QuizQuestionLengths[i][1]; j++){
                currentQuizNumbers.push(j)
            }
            break
        }
    }
    console.log('currentQuizNumbers',currentQuizNumbers)
    setCurrentQuizNumbers(currentQuizNumbers)
    return(currentQuizNumbers)
}

console.log('TestURL',decodeBase85(TestURL))
/*
const dropdownQuestion = document.getElementById('quiz-question-content');
QuizQuestionLengths.forEach(item => {
    const option = document.createElement('a');

    option.textContent = item[1]; // Display the name instead of the number
    dropdownQuestion.appendChild(option);
});

dropdownQuestion.addEventListener('change', (event) => {
        QuizNumber = event.target.value;
        console.log(`Selected Value: ${QuizNumber}`);
    });

document.addEventListener("DOMContentLoaded", function() {
        const nextQuestionButton = document.getElementById("next-question-btn");
        const dropdown = document.querySelector(".dropdownQuizQuestion");
    
        // Some condition. For the sake of this example, I'm using a boolean variable.
        
    
        if (currentType == 'Practice') {
            nextQuestionButton.style.display = "hidden";
...
toggleBtn.addEventListener('click', function() {
        if (targetDiv.style.display === 'none' || targetDiv.classList.contains('hidden')) {
...
toggleCalcBtn.addEventListener('click', function() {
    if (calculatorDiv.style.display === 'none' || calculatorDiv.classList.contains('hidden')) {
        calculatorDiv.style.display = 'block';
        calculatorDiv.classList.remove('hidden');
        //toggleCalcBtn.textContent = 'Hide Calculator';
    } else {
        calculatorDiv.style.display = 'none';
        //toggleCalcBtn.textContent = 'Show Calculator';
    }
});
...
function setImageDataUrl(newValue) {
    imageDataUrl = newValue;
    // Add logic to update UI as needed
}
function setCorrectAnswerChoicePractice(newValue) {
    correctAnswerChoicePractice = newValue;
    // Add logic to update UI as needed
}

function setExplanation(newValue) {
    explanation = newValue;
    // Add logic to update UI as needed
}

function setCurrentQuestionInfo(newValue) {
    currentQuestionInfo = newValue;
    // Add logic to update UI as needed
}

function setCurrentShaCode(newValue) {
    currentShaCode = newValue;
    // Add logic to update UI as needed
}

function setQuestionNeedsReview(newValue) {

    console.log('setQuestionNeedsReview',newValue)
    QuestionNeedsReview = newValue;
    // Add logic to update UI as needed
}
...
// Add an event listener to detect changes (when it's checked or unchecked)
checkbox.addEventListener('change', function() {
      if(this.checked) {
          setQuestionNeedsReview(true)
      } else {
            setQuestionNeedsReview(false)
      }
  });

function setCurrentPhaseInQuestion(newValue) {
    currentPhaseInQuestion = newValue;
    // Add logic to update UI as needed
    // Fetch new image data when the phase or topic changes
...
function setCurrentLocalTopicForQuestions(newValue) {
    currentLocalTopicForQuestions = newValue;
    // Add logic to update UI as needed
    // Fetch new image data when the phase or topic changes


    QuizQuestionLengthsFunction(newValue)
   
}


 // Your array
 let first_elements = ['Absolute value', 'Expressions', 'Inequalities', 'Lines', 'Solving algebraic equations', 'Systems of equations', 'Word problems', 'Complex numbers', 'Constructing models', 'Exponents and radicals', 'Exponential and linear growth', 'Functions', 'Matching coefficients', 'Quadratics', 'Synthetic division', 'Experiment design', 'Mean, median and mode', 'Percents', 'Probability', 'Ratio and proportion', 'Reading data', 'Scatter plots', 'Angles', 'Circles', 'Triangles', 'Trigonometry', 'Volume', 'Add, revise, or delete', 'Apostrophes', 'Combining and separating sentences', 'Diction, idioms, and register', 'Modification', 'Non-essential and essential clauses', 'Parallel structure', 'Pronoun and noun agreement', 'Punctuation', 'Sentence and paragraph order', 'Sentences and fragments', 'Shorter is better', 'Infographics', 'Transitions', 'Verb agreement and tense', 'Word pairs and comparisons', 'Fiction', 'Social Science', 'Natural Science', 'Historical Documents', 'Paired Passages'];
 var second_elemnts = ['Word problems', 'Absolute value', 'Expressions', 'Inequalities', 'Solving equations', 'Systems of equations', 'Complex numbers', 'Exponents and radicals', 'Numbers and Operations', 'Properties of numbers', 'Functions', 'Coordinate Geometry', 'Quadratics', 'Logaritms', 'Matrices', 'Sequences', 'Percents', 'Probability', 'Ratio and proportion', 'Vectors', 'Area and Perimeter', 'Angles', 'Circles', 'Triangles', 'Trigonometry', 'Volume', 'Ellipses', 'Data and Statistics', 'Add, revise, or delete', 'Apostrophes', 'Joining and separating sentences', 'Diction, idioms, and register', 'Modification', 'Non-essential and essential clauses', 'Parallel structure', 'Pronouns', 'Punctuation', 'Sentence and paragraph order', 'Sentences and fragments', 'Shorter is better', 'Transitions', 'Verbs', 'Word pairs and comparisons', 'Evaluation of Purpose', 'Adjectives and Adverbs']
...
if(MathrowsGlobal.length > 0 || VerbalrowsGlobal.length > 0 || ReadingrowsGlobal.length > 0 || ScienceRowsGlobal.length > 0){
    var combinedArr = MathrowsGlobal.concat(VerbalrowsGlobal, ReadingrowsGlobal, ScienceRowsGlobal);
    if(currentTest == 'SAT'){
        for (var i = 0; i < combinedArr.length; i++) {
            sat_percentages.push(combinedArr[i].Percent);
        }
    } else if(currentTest == 'ACT'){
        for (var i = 0; i < combinedArr.length; i++) {
            act_percentages.push(combinedArr[i].Percent);
        }
    }
}

let practice_elements = ['Practice', 'Quiz']

 let act_elements = second_elemnts;
 let sat_elements = first_elements;
 

// Initialize with SAT elements
updateDropdownContent(sat_elements, sat_percentages, currentTest, currentType, currentQuizNumbers);

// Setup test selector dropdown
let testSelector = document.getElementById('test-selector');
let typeSelector  = document.getElementById('type-selector');
let quizQuestionSelector = document.getElementById('quiz-question-selector');



let actOption = document.createElement('a');
actOption.href = "#";
actOption.textContent = 'ACT';
actOption.onclick = function() {
  updateDropdownContent(act_elements, act_percentages, 'ACT',currentType, QuizNumber);
}

let satOption = document.createElement('a');
satOption.href = "#";
satOption.textContent = 'SAT';
satOption.onclick = function() {
  updateDropdownContent(sat_elements, sat_percentages, 'SAT', currentType, QuizNumber);
}

let practiceOption = document.createElement('a');
practiceOption.href = "#";
practiceOption.textContent = 'Practice';
practiceOption.onclick = function() {
    if(currentTest == 'SAT'){
        updateDropdownContent(sat_elements, sat_percentages, currentTest, 'Practice', QuizNumber);
    } else if(currentTest == 'ACT'){
        updateDropdownContent(act_elements, act_percentages, currentTest, 'Practice', QuizNumber);
    }
}

let quizOption = document.createElement('a');
quizOption.href = "#";
quizOption.textContent = 'Quiz';
quizOption.onclick = function() {
    if(currentTest == 'SAT'){
        updateDropdownContent(sat_elements, sat_percentages, currentTest, 'Quiz', QuizNumber);
    } else if(currentTest == 'ACT'){
        updateDropdownContent(act_elements, act_percentages, currentTest, 'Quiz', QuizNumber);
    }
}
...
typeSelector.appendChild(practiceOption);

typeSelector.appendChild(quizOption);


testSelector.appendChild(actOption);
testSelector.appendChild(satOption);



let buttonPressed = false;  // Variable to store button state

// Get the button element
let buttonX = document.getElementById('next-question-btn');

// Add a 'click' event listener to the button
buttonX.addEventListener('click', function() {
    buttonPressed = !buttonPressed;  // Toggle the button state
    console.log('Button has been pressed:', buttonPressed);
    setCurrentPhaseInQuestion(currentPhaseInQuestion + 1)
});
// Initially disable the button
buttonX.disabled = true;

// Let's say this is your condition
let condition = false;
...
// Function to update dropdown contents
function updateDropdownContent(elements, percentages, test, type, quizQuestion) {
    
    

    

    document.getElementById('type-selector-btn').textContent = type;

    document.getElementById('test-selector-btn').textContent = test;



    document.getElementById('quiz-question-btn').textContent = "Question Number: "+ (quizQuestion + 1).toString();

    var nextQuestionButton = document.getElementById("next-question-btn");
    var dropdown = document.getElementById("dropdownQuizQuestion");
    ...
    currentTest = test
    currentType = type
    QuizNumber = quizQuestion
    let dropdownContent = document.getElementById('dropdown-content');
    let dropdownQuizQuestionContent = document.getElementById('quiz-question-content');
    
    // First, clear out old elements
    while (dropdownContent.firstChild) {
      dropdownContent.removeChild(dropdownContent.firstChild);
    }
    
    while (dropdownQuizQuestionContent.firstChild) {    
        dropdownQuizQuestionContent.removeChild(dropdownQuizQuestionContent.firstChild);
    }
    console.log("adding new stuff")
    console.log("currentQuizNumbers", currentQuizNumbers)

    // Then, create and append new elements
    for(let j = 0; j < currentQuizNumbers.length; j++){
        let a = document.createElement('a');
        a.href = "#";
        a.textContent = currentQuizNumbers[j] + 1;
        if(currentQuizNumbers[j] == QuizNumber){
            a.style.color = "black";
        }
        a.onclick = function() {
            document.getElementById('quiz-question-btn').textContent =  (currentQuizNumbers[j] + 1).toString();
            QuizNumber = currentQuizNumbers[j]
            console.log(`Selected Value: ${QuizNumber}`);
            if(currentTest == 'SAT'){
                updateDropdownContent(sat_elements, sat_percentages, currentTest, currentType, QuizNumber);
            } else if(currentTest == 'ACT'){
                updateDropdownContent(act_elements, act_percentages, currentTest, currentType, QuizNumber);
            }
        }
     
        dropdownQuizQuestionContent.appendChild(a);
    }
    console.log('dropdownQuizQuestionContent', dropdownQuizQuestionContent)


    for (let i = 0; i < elements.length; i++) {
      let a = document.createElement('a');
      a.href = "#";
      a.textContent = elements[i];
  
      // Only add percentage if data is available
      if (percentages && percentages[i] != null) {
      ...
      a.onclick = function() {
        document.getElementById('dropdown-btn').textContent = elements[i];
        //condition = true; // This would be where your actual condition-checking logic goes
        //checkCondition();
        console.log("Current topic: " + elements[i]);
        setCurrentLocalTopicForQuestions(elements[i])
        fetchImageData()
      }
...
function sendShaCodeToFirebase(shaCode, sentiment) {
    function FindMatchingId(ID){
      // Assuming NameId is globally defined
    ...
    // Assuming auth and db are globally defined
    var studentDef = firebase.firestore().doc("users/" + FindMatchingId(auth.currentUser.uid));
  
    if(studentDef){
      firebase.firestore().runTransaction((transaction) => {
        return transaction.get(studentDef).then((sfDoc) => {
          if (!sfDoc.exists) {
            throw "Document does not exist!";
          }
    
          var newStudents = sfDoc.data().Students || [];
          newStudents.push(shaCode+ "-" + sentiment + "-" + today);
          transaction.update(studentDef, { Students: newStudents });
        });
      }).catch((error) => {
...
function clearObjectsExceptBackground() {
    // Get all objects
    var objects = wbCanvas.getObjects();

    // Remove each object
    for (var i = objects.length - 1; i >= 0; i--) { // iterate backwards to safely remove items
        if(!objects[i].isType('image') || !objects[i].isBgImage) {
            wbCanvas.remove(objects[i]);
        }
    }
    wbCanvas.renderAll();
}
...
function UseBackgroundImage(imgURL){
    fabric.Image.fromURL(imgURL, function(img) {
    // add background image
    wbCanvas.setBackgroundImage(img, wbCanvas.renderAll.bind(wbCanvas), {
            scaleX: wbCanvas.width / img.width,
            scaleY: wbCanvas.height / img.height,
            isBgImage: true
        });
    });
}
  ...
  if(currentLocalTopicForQuestions != ''){
    ...
    return fetch(url)
        ...
        .then(data => {
            ...
            UseBackgroundImage(blobUrl)
            setExplanation(data.question_data.lesson_plan)
       
  
            var CurrJSON=JSON.parse(data.question_data.lesson_plan)
            document.getElementById("learningObjective").textContent = CurrJSON.learningObjective;
            document.getElementById("topic").textContent = CurrJSON.topic;
            document.getElementById("answer").textContent = data.question_data.answer;
            const instructionsList = document.getElementById("instructionsList");
            CurrJSON.instructions.forEach(instruction => {
        ...
        console.log('newURL')
        console.log(newURL)
        const url = `http://localhost:5005/usr/${uid}%${newURL}%${true}%${QuizNumber}`;
...
}, [CurrentPhaseInQuestion,currentLocalTopicForQuestions]);
        ...
        case 'imgUrl':
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    console.log('Image URL: ', wbCanvasImgURL)
